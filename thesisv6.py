...
# --- NEW CONFIGURATION FOR LOGIC ---
POST_ENTRY_DELAY = 0.5   # Seconds to wait after car clears sensor
EXIT_SCAN_COOLDOWN = 5.0 # Seconds to wait for new plate before closing
...
def log_to_gui_and_csv(plate, name, faculty, status, latency):
    ...
    try:
        ...
        # LOGIC: Only trigger gate if authorized
...
def trigger_gate_sequence():
    ...
    # CASE 1: Gate is ALREADY open and in "POST-ENTRY SCAN" mode (Successive Vehicle)
    if is_gate_busy and system_state == "POST-ENTRY SCAN":
        ...
        vehicle_confirmed = False
        vehicle_entry_start_time = None
        # Restart the sensor check loop immediately (switch back from timer to sensor)
        root.after(100, smart_gate_check)
        return

    # CASE 2: Gate is busy moving (Opening/Closing) - Ignore
    if is_gate_busy: return 

    # CASE 3: Standard Entry (Gate Closed)
    is_gate_busy = True
    ...
    # Wait for physical open, then start sensor check
...
def smart_gate_check():
    ...
    # Stop this loop if we switched to Post-Entry Scan (handled by separate timer)
...
# --- NEW HELPER FUNCTIONS FOR DELAY & SCAN ---

def start_post_entry_wait():
    """Waits 0.5s after car leaves before scanning again."""
    set_status("WAITING...", "yellow")
    # Wait POST_ENTRY_DELAY seconds, then start scanning
    root.after(int(POST_ENTRY_DELAY * 1000), lambda: init_post_entry_scan())

def init_post_entry_scan():
    """Sets state to scan for next car."""
    global system_state, last_plate_seen_time
    system_state = "POST-ENTRY SCAN"
    # Reset the timer so we have a fresh 5 seconds
    last_plate_seen_time = time.time() 
    monitor_post_entry_timer()

def monitor_post_entry_timer():
    """Checks if 5 seconds have passed without seeing a car."""
    global system_state
    
    # If state changed (e.g., trigger_gate_sequence found a new car), stop this timer
    if system_state != "POST-ENTRY SCAN": return

    # Check how long since update_frame saw a box
...
def update_frame():
    ...
    # LOGIC UPDATE: Allow detection if we are SCANNING *OR* in POST-ENTRY SCAN mode
