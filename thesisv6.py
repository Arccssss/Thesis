...
GATE_CLOSE_GRACE_TIMEOUT = 5.0  # 5-second scanning window before closing
POST_ENTRY_SCAN_DELAY = 0.5     # 0.5s pause after car clears sensor before scanning again
...
def update_frame():
    ...
    if is_in_delay: cv2.putText(frame, "READYING NEXT SCAN...", (30, 140), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 100, 0), 2)
    elif is_waiting_for_car: cv2.putText(frame, "GATE OPEN: AWAITING PASSAGE", (30, 140), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    elif plate_absence_start_time and is_gate_busy:
        rem = max(0, GATE_CLOSE_GRACE_TIMEOUT - (curr_t - plate_absence_start_time))
        cv2.putText(frame, f"GRACE: {rem:.1f}s", (30, 140), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 165, 255), 2)

    # STRICT SCANNING LOGIC
    # AI stops if: closing gate OR waiting for confirmed car OR in the 0.5s ready delay
            ...
            if len(boxes) > 0:
                ...
                for b in boxes:
                    ...
                    if (curr_t - detection_start_time) > GRACE_PERIOD:
                        ...
                        if p_crop.size > 0:
                            # Preprocess internally for EasyOCR
                            gray = cv2.cvtColor(p_crop, cv2.COLOR_RGB2GRAY)
