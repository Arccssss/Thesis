...
ABSENCE_RESET_TIME = 2.5 # NEW: Seconds to wait before resetting latency if no car is seen
...
last_plate_seen_time = 0 # NEW: Tracks when we last saw a box
is_gate_busy = False
    ...
    if not is_gate_busy and system_state in ["SCANNING", "DETECTING CAR", "UNAUTHORIZED DETECTED"]:
        ...
        for result in results:
            ...
            if len(boxes) > 0:
                ...
                # --- NEW LATENCY LOGIC START ---
                # Update the last time we saw a plate
                last_plate_seen_time = current_time 
                
                # If this is the start of a session, initialize
                if detection_start_time is None: 
                    detection_start_time = current_time
                
                time_seen = current_time - detection_start_time
                # --- NEW LATENCY LOGIC END ---
                            ...
                            if len(clean_text) > 3: 
                                ...
                                if len(scan_buffer) > 0:
                                    ...
                                    if (cur_t - last_log) > LOG_COOLDOWN:
                                        ...
                                        # Use system latency instead of per-string if preferred, 
                                        # but per-string is usually safer for mixed traffic.
        ...
        # --- ABSENCE RESET LOGIC ---
        # If no box detected in THIS frame
        if not detected_box:
            # Check if it has been absent for longer than the threshold
