...
from gpiozero import OutputDevice, DistanceSensor, Buzzer, LED  # Added LED here
...
# --- NEW LED PINS ---
LED_OPEN_PIN = 5   # Lights up when OPENING/OPEN
LED_CLOSE_PIN = 6  # Lights up when CLOSING/CLOSED

# ==========================================
#         HARDWARE INITIALIZATION
# ==========================================
try:
    # Initialize Gate Motors (Closed state: P1 Off, P2 On)
    ...
    # Set Initial LED State (System starts Closed)
    led_open.off()
    led_close.on()
    
    # Picamera2 Initialization for RPi 5
    picam2 = Picamera2()
    ...
    # Dummy Device for error handling (Used for Motors, Buzzer, and LEDs)
    class DummyDev: 
        def on(self): pass
        def off(self): pass
    ...
    try:
        with open(LOG_FILE, 'r') as f:
            ...
            for row in reversed(rows):
                ...
                try:
                    dt_obj = datetime.strptime(full_ts, "%Y-%m-%d %H:%M:%S")
                    short_time = dt_obj.strftime("%H:%M:%S")
                except:
                    short_time = full_ts
...
def smart_gate_check():
    ...
    try:
        ...
        if dist_cm < SAFETY_DISTANCE_CM:
            if system_state != "CLOSING GATE" and system_state != "UNAUTHORIZED DETECTED":
...
def update_frame():
    ...
    if system_state in ["SCANNING", "DETECTING CAR", "UNAUTHORIZED DETECTED"]:
                ...
                for box in boxes:
                    ...
                    if time_seen > GRACE_PERIOD:
                        ...
                        if p_crop.size > 0:
                            ...
                            text = "".join(ocr_res).replace(" ", "").upper()
    ...
    if img_h > 0: aspect_ratio = img_w / img_h
    else: aspect_ratio = 1.77 
    ...
    try:
        ...
        img = Image.fromarray(cv2.cvtColor(img_small, cv2.COLOR_BGR2RGB))
...
try:
    ...
    gate_p1.close()
    gate_p2.on()
    led_open.off()  # Turn off open LED
    led_close.on()  # Ensure closed LED is on (or both off if preferred)
